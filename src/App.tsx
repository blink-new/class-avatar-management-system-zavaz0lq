...
function App() {
  ...
  const loadUsers = async () => {
    try {
      const dbUsers = await blink.db.users.list({ orderBy: { points: 'desc' } });
      if (dbUsers.length > 0) {
        const formattedUsers = dbUsers.map((dbUser, idx) => ({
          id: dbUser.id + '-' + idx,
          email: dbUser.email,
    ...
    try {
      ...
      if (stored) {
        ...
        setUsers(storedUsers.map((u: User, idx: number) => ({ ...u, id: u.id + '-' + idx })));
        return;
      }
    } catch (error) {
      console.warn('localStorage not available:', error);
    }
    setUsers(mockUsers.map((u, idx) => ({ ...u, id: u.id + '-' + idx })));
  ...
  const loadPointTransactions = async () => {
    try {
      const dbTransactions = await blink.db.pointTransactions.list({ orderBy: { createdAt: 'desc' }, limit: 50 });
      const formattedTransactions = dbTransactions.map((tx, idx) => ({
        id: tx.id + '-' + idx,
        userId: tx.userId,
      ...
      if (state.user) {
        ...
        if (existingUser) {
          ...
          currentUser = {
            ...
            displayName: '', // force username modal
            role: 'student',
            points: 0,
  ...
  const handleSaveUsername = (username: string) => {
    ...
    try {
      blink.db.users.update(user.id, { displayName: username });
    } catch {}
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-100 via-pink-50 to-cyan-100">
      ...
      <UsernameModal open={showUsernameModal || !user.displayName} onSave={handleSaveUsername} />
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-6">
                    ...
                    <Card className="bg-white/80 backdrop-blur-sm">
                      ...
                      <CardContent>
                        {pointTransactions.slice(0, 5).map((transaction, idx) => {
                          const student = users.find(u => u.id === transaction.userId);
                          return (
                            <div key={transaction.id + '-' + idx} className="flex items-center justify-between py-3 border-b border-gray-100 last:border-b-0">
